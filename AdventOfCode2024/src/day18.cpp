...
const std::vector<Direction> directions = {{0, 1}, {-1, 0}, {1, 0}, {0, -1}};
...
int bfs(const std::set<Position> &positions, const Position &startPos,
  ...
  while (!q.empty()) {
    ...
    for (const auto &[dx, dy] : directions) {
      ...
      // std::cout << " -- try push " << newX << ", " << newY << std::endl;
      const bool isInMap =
      ...
      if (isInMap && !isBlocked && !isVisited) {
        ...
        // std::cout << " -- push " << newX << ", " << newY << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  // auto resultP1 = solverP1(input, 12, {0, 0}, {6, 6});
  // std::cout << "Test P1 result: " << resultP1 << std::endl;

  // auto resultP2 = solverP2(input, {0, 0}, {6, 6});
  // const auto &[x, y] = resultP2;
  // std::cout << "Test P2 result: " << x << "," << y << std::endl;
